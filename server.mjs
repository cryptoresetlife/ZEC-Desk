...
const syncer=setInterval(()=>{if(wallet.ready&&!engine.busy&&!closing)void wallet.command('sync',['run']).catch(()=>{});},20000);
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(req.headers['x-zec-desk']!==token || (req.headers.origin&&req.headers.origin!==origin))return json(res,403,{error:'请从本机软件窗口操作'});
    ...
    if(path==='/api/wallet/status'){await wallet.status();return json(res,200,{ok:true});}
    ...
    if(path==='/api/exit'){
      if(engine.busy||wallet.starting)throw new Error('正在启动钱包、核对或发送交易，请等本次操作结束再退出');
