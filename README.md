...
- Grok 手动研究：官方设备授权登录，或自行填写 xAI API Key；结果提供来源链接。
...
2. 在“项目雷达 / 市场监控”启动扫描；在“自选项目”粘贴 HTTPS 官网链接，预览后添加。
3. 如需钱包与自动 mint，安装 WSL 的 **Ubuntu** 发行版（建议 Ubuntu 24.04）：管理员终端运行 `wsl --install -d Ubuntu`，按提示重启、完成 Ubuntu 首次初始化。
4. 在“独立钱包”创建或打开软件钱包，备份恢复信息，再转入自己决定的少量预算。它与 Noir 插件钱包独立。
5. 在“自动 mint”核对项目、收款规则、数量、单价和费用预算，完成预检后确认启动。保持电脑开机且不休眠。
6. 退出请用“停止并退出软件”。关闭窗口后后台可能继续运行；重新打开不会自动恢复付款任务。
...
- 扫描和手动研究不需要 WSL；自动 mint 钱包需要它。程序未做代码签名。
