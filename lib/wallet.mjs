...
export function walletScanComplete(sync){
  return Number.isSafeInteger(sync?.sync_start_height)&&sync.sync_start_height>0&&Array.isArray(sync.scan_ranges)&&sync.scan_ranges.length>0&&sync.scan_ranges.every(r=>r.priority==='Scanned');
}
export function syncFailure(raw){
  ...
  if(/Inserted root conflicts with existing root/i.test(text)){
    const error=new Error('钱包扫描记录与区块数据冲突，同步已暂停。需要先备份钱包并重建扫描记录；余额尚未核实，重复刷新不能解决。');
    error.syncCode='tree-conflict';return error;
  }
  if(/rescan required|missing .*shard tree checkpoints/i.test(text)){
    const error=new Error('钱包扫描检查点缺失，同步已暂停。需要先备份钱包并重新扫描；当前余额尚未核实。');
    error.syncCode='rescan-required';return error;
  }
    ...
    try{
      if(!this.syncBlocked&&Date.now()>=this.syncRetryAt){
        try{
          ...
          this.syncBlocked=['unsupported-tx-format','tree-conflict','rescan-required'].includes(this.syncCode);
          this.syncError=this.syncBlocked?e.message:'钱包同步任务失败，将稍后重试。请检查网络和轻钱包服务连接。';
