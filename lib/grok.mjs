...
async function readJson(r){
  ...
  if(!r.body)throw Error('empty');
  for await(const c of r.body){bytes+=c.length;if(bytes>2000000)throw Error('too large');text+=decoder.decode(c,{stream:true});}
  return JSON.parse(text+decoder.decode());
}
function denied(status){return status===401?'登录或密钥已失效，请重新连接':status===403?'账号没有此接口或模型的权限；Premium 标签不代表已授权':status===402?'当前通道额度不足，请在官方账户检查':status===426?'服务要求更新客户端兼容协议（HTTP 426）；不能据此判断订阅额度':status===429?'当前通道限流，请稍后再试':'服务暂不可用（HTTP '+status+'）';}
export function researchResult(d){
  if(d?.error||d?.status!=='completed'||!Array.isArray(d.output))throw Error('incomplete');
  ...
  if(!texts.length)throw Error('no text');
...
export class GrokConnection {
  ...
  #mode='subscription';#busy=false;#polling=false;#models=[];#checkedAt=0;#error='';#result=null;#nextRun=0;
  constructor({fetcher=fetch,now=Date.now}={}){this.fetcher=fetcher;this.now=now;}
  view(){const p=this.#pending;return {mode:this.#mode,loggedIn:!!this.#access,keyReady:!!this.#key,busy:this.#busy,models:this.#models,checkedAt:this.#checkedAt,error:this.#error,result:this.#result,nextRun:this.#nextRun,entitlement:'以实际接口返回为准；尚未核实 X 订阅等级',login:p?{url:p.url,code:p.userCode,expiresAt:p.expiresAt}:null};}
  ...
  async #fetch(url,opts={},timeout=15000){
    return this.fetcher(url,{...opts,redirect:'error',signal:AbortSignal.any([this.#controller.signal,AbortSignal.timeout(timeout)])});
  }
  ...
  async #request(path,body,g){
    ...
    const r=await this.#fetch(BASE[this.#mode]+path,{headers,...(body?{method:'POST',body:JSON.stringify(body)}:{})},body?60000:15000);
    if(!r.ok){await r.body?.cancel();throw Error(denied(r.status));}
    return readJson(r);
  }
  async #run(action){
    if(this.#busy)throw Error('正在处理 Grok 请求，请稍候');this.#busy=true;this.#error='';const g=this.#generation;
    try{await action(g);}catch(e){if(g===this.#generation){this.#error=/^(登录|请先|账号没有|当前通道|服务暂|服务要求更新客户端兼容协议|未获得|请填写|请先选择|调用间隔)/.test(e.message)?e.message:'Grok 请求未完成；保留上次结果，请检查连接或权限';throw Error(this.#error);}}
    finally{this.#busy=false;}
  }
