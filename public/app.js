...
bind('#taskopen',async()=>{const t=selectedTask();if(!window.chrome?.webview?.postMessage)throw Error('内置任务窗口需要新版 ZEC Desk.exe。当前可使用“在浏览器打开”并复制钱包地址。');if(taskOpenPending)throw Error('正在打开上一项目，请稍等');taskOpenPending=t;window.chrome.webview.postMessage({type:'project-task.open',url:t.url,name:t.name,address:t.w?.address||''});setTimeout(()=>{if(taskOpenPending===t){taskOpenPending=null;say('未收到桌面窗口响应。请关闭软件窗口，再从原文件夹的 ZEC Desk.exe 打开新版界面；后台无需停止。');}},4000);say('正在打开项目任务窗口…');});
