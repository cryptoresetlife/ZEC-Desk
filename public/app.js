...
let state=null,preview=null,dialogAction=null,mutating=false;
...
function dialog(title,content,action,confirm='确认'){ $('#dialogtitle').textContent=title;$('#dialogbody').innerHTML=content;dialogAction=action;$('#dialogconfirm').textContent=confirm;$('#dialogconfirm').hidden=!action;$('#dialog').showModal();}
$('#dialogclose').onclick=()=>{$('#dialog').close();$('#dialogbody').textContent='';dialogAction=null;};
$('#dialog').addEventListener('close',()=>{$('#dialogbody').textContent='';dialogAction=null;});
bind('#dialogconfirm',async()=>{if(dialogAction)await dialogAction();$('#dialog').close();});
...
bind('#backup',async()=>{dialog('本机查看备份','<p>下一步会在本机显示助记词。请离线保管，不要发到聊天、GitHub 或截图中。</p>',async()=>{const d=await api('wallet/backup',{});setTimeout(()=>dialog('备份独立钱包','<pre>'+esc(typeof d.recovery==='string'?d.recovery:JSON.stringify(d.recovery,null,2))+'</pre><p>请记下完整助记词与生日区块。关闭后清除本窗口显示。</p>',()=>api('wallet/backed-up',{confirm:true}),'已离线保存备份'),0);},'在本机显示');});
...
// Artwork is owned by the project; the release does not redistribute its cache.
...
function paint(s){
 ...
 $('#walletsync').textContent=typeof s.wallet.sync?.percentage_total_outputs_scanned==='number'?'钱包扫描进度：'+s.wallet.sync.percentage_total_outputs_scanned.toFixed(1)+'% · 充值需等待至少 3 个确认。':'充值后需等待链上确认与同步完成。';
...
function paintLaunches(r){
 ...
 setCards('#launchlist',list.map(x=>'<article class="market-card">'+thumb(x.image,x.name)+'<h3><a href="'+esc(x.url)+'" target="_blank" rel="noreferrer">'+esc(x.name)+' ↗</a></h3><span class="badge">'+esc(names[x.phase]||names.unknown)+'</span><p>'+esc(x.note)+'</p><small>'+esc(x.source)+(x.checkedAt?' · '+esc(when(x.checkedAt)):'')+'</small>'+(x.start?'<p>Public 时间：'+esc(when(x.start))+' · 单价 '+esc(x.price??'待核实')+' ZEC</p>':'')+'<div class="row">'+(x.autoMint?'<button data-launch-mint="'+esc(x.id)+'">设置自动 mint</button>':x.phase!=='market'?'<button data-launch-plan="'+esc(x.id)+'">'+(r.plans.some(p=>p.id===x.id)?'已加入待开售':'加入待开售计划')+'</button>':'<span>请先添加项目官网</span>')+'</div></article>').join('')||'<p>当前筛选没有结果；不代表全链没有开售项目。</p>');
 setCards('#launchplans',r.plans.map(p=>'<article><h3>'+esc(p.name)+'</h3><p>'+(p.status==='needs-budget'?'待设置预算并启动':'等待适配：目前仅监控，未启动自动付款')+'</p><div class="row"><a href="'+esc(p.url)+'" target="_blank" rel="noreferrer">打开官网 ↗</a>'+(p.id==='zaddr'?'<button data-launch-mint="zaddr">设置自动 mint</button>':'')+'<button class="ghost" data-launch-remove="'+esc(p.id)+'">移除计划</button></div></article>').join('')||'<p>暂无待开售计划。</p>');
...
for(const area of ['#launchlist','#launchplans'])$(area).addEventListener('click',async e=>{
 ...
 btn.disabled=true;try{
  ...
  else {const id=btn.dataset.launchMint||btn.dataset.launchPlan;await api('launches/plan',{id});if(btn.dataset.launchMint){$('#mint').scrollIntoView({behavior:'smooth'});say('已选择 ZADDR Public。请填写预算、检查条件并确认启动；当前尚未启动付款。');}else say('已加入待开售计划，目前只监控；适配完成并确认预算后才能自动 mint。');}
  await refresh();
